...
function formatRelativeDate(isoDate: string): string {
  ...
  if (diffHours < 0) return 'Ya publicado';
  if (diffHours < 24) return `Hoy a las ${date.toLocaleTimeString('es-PR', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'America/Puerto_Rico' })}`;
  if (diffDays === 1) return `Mañana a las ${date.toLocaleTimeString('es-PR', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'America/Puerto_Rico' })}`;
  
  const dayNames = ['dom', 'lun', 'mar', 'mié', 'jue', 'vie', 'sáb'];
  const prDate = new Date(date.toLocaleString('en-US', { timeZone: 'America/Puerto_Rico' }));
  return `${dayNames[prDate.getDay()]} ${prDate.getDate()}/${prDate.getMonth() + 1} a las ${date.toLocaleTimeString('es-PR', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'America/Puerto_Rico' })}`;
}
...
function PerformanceMini() {
  ...
  return (
    <div className="mt-3">
      ...
      {/* Next scheduled post */}
      {data.nextPost && (
        <div className="mb-2 p-2.5 rounded-lg bg-[var(--pioneer-teal-bg)] border border-[var(--pioneer-teal)]/20">
          <div className="flex items-center gap-1.5 mb-1">
            <CalendarIcon />
            <span className="text-[10px] font-medium text-[var(--pioneer-teal)] uppercase tracking-wide">Próximo post</span>
          </div>
          <p className="text-xs font-medium text-foreground leading-tight truncate">{data.nextPost.title}</p>
          <p className="text-[10px] text-muted-foreground mt-0.5">{formatRelativeDate(data.nextPost.scheduledFor)}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="p-2.5 rounded-lg bg-muted/50 border border-border/50">
          <p className="text-lg font-bold text-foreground leading-none">{data.scheduledPosts}</p>
        ...
        {data.totalImpressions > 0 && (
          <div className="p-2.5 rounded-lg bg-muted/50 border border-border/50">
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">Vistas</p>
          </div>
        )}
        {data.totalLikes > 0 && (
          <div className="p-2.5 rounded-lg bg-muted/50 border border-border/50">
        ...
        {data.totalComments > 0 && (
          <div className="p-2.5 rounded-lg bg-muted/50 border border-border/50">
            <p className="text-lg font-bold text-[var(--pioneer-teal)] leading-none">{formatNum(data.totalComments)}</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">Comentarios</p>
          </div>
        )}
        {data.totalShares > 0 && (
          <div className="p-2.5 rounded-lg bg-muted/50 border border-border/50">
            <p className="text-lg font-bold text-[var(--pioneer-teal)] leading-none">{formatNum(data.totalShares)}</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">Compartidos</p>
          </div>
        )}
        {data.totalClicks > 0 && (
          <div className="p-2.5 rounded-lg bg-muted/50 border border-border/50">
            <p className="text-lg font-bold text-[var(--pioneer-teal)] leading-none">{formatNum(data.totalClicks)}</p>
